...
- **Published & consumable:** plugin `openclaw-webchannel@0.1.0` on **ClawHub**
  ...
  on the **public npm registry** since 0.6.1 — zero-auth install, OIDC trusted publishing with
  provenance (tag-triggered `publish.yml`, lockstep versioning — see
  `docs/PUBLISHING.md`). Public-API-only consumers exist and are CI-gated:
...
| Example app is not a scaffold | The registry half of this gap is closed — `@mir-stream/*` moved to public npm in 0.6.1 and needs no token — but the example app is still not a standalone `npm create` scaffold. |
