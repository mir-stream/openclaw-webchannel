...
- 레지스트리 실설치(§10) — 0.6.1부터 퍼블릭 npm 이라 토큰은 필요 없지만 이 계획의 범위 밖.
...
## 10. 리스크 / 오픈 이슈
- **레지스트리 실설치는 별도 단계**: 패키지명 import는 워크스페이스 심링크→exports 맵으로 해석되어 게시
  계약과 동일 검증. 0.6.1부터 두 패키지가 퍼블릭 npm 이라 토큰 게이트는 사라졌고, 실제 `npm i`로의 최종
  확인은 여전히 별도 단계(현재 비목표).
