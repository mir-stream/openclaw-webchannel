...
4. **`kind:"final"`은 메시지 식별자가 아니라 core의 최종-payload 전달 분류다.** 한 턴에 error/answer/warning뿐 아니라 이미 materialize된 assistant text의 replay도 올 수 있다. terminal notice는 assistant lane을 소비하지 않고, retained-answer replay는 기존 lane ID에 구조적으로 재적용하며, 정말 상관되지 않는 payload만 보존 fallback으로 append한다. 앞 메시지와 합칠지 텍스트 내용으로 추측하지 않는다.
...
| `delivery.deliver(..., { kind:"final" })` | core 최종-payload 배열의 한 원소다. 한 턴에 여러 번 올 수 있고 `kind`만으로 assistant-message 소유권을 알 수 없다. 지원 하한의 terminal-error 경로는 `[error, ...retained assistantTexts]`를 만들 수 있다. | notice/error는 별도 ID로 보존하고 assistant lane을 소비하지 않는다. ordinary answer는 current lane을 정착하고, leading-error 뒤 retained-answer replay는 generation 순서의 기존 lane ID에 재적용한다. 구조적으로 상관할 수 없는 payload만 진단 후 fresh fallback으로 보존한다. |
...
`kind:"final"`의 다중성과 replay는 가정이 아니라 지원 하한 `2026.6.10`의 관측 가능한 동작이다. `replyResult`가 배열이면 dispatcher가 각 원소를 같은 `kind:"final"` delivery seam으로 순서대로 보낸다. 고정 builder는 errored run에서 먼저 `errorText`를 넣고, canonical single-answer 경로를 쓸 수 없으면 보존된 `assistantTexts`를 앞에서부터 모두 뒤에 넣는다. 이미 A/B lane이 스트리밍된 턴도 `[error, A, B]`가 될 수 있다는 뜻이다. 이 저장소도 terminal error → retained answer와 answer → timeout/tool warning을 `inbound.test.ts`에서 모델링하지만, 기존 fixture가 `streaming.mode:"off"`라 draft settle/replay 중복을 검출하지 못한다.

delivery 이음매에는 payload index나 배열 시작/끝 표지가 없으므로 **임의의 미래 final 배열을 완전하게 상관할 수 있다는 계약은 없다.** 이 계획은 공개 payload flags와 지원 하한에서 검증한 두 구조를 분리한다. (a) ordinary answer final은 current lane, (b) answer보다 먼저 온 terminal error는 retained-answer replay phase를 열고 이후 non-notice answer payload를 generation 순서의 lane에 재적용한다. 그 밖의 모호한 추가 non-error final은 기존 버블을 내용으로 dedupe하지 않고 진단 후 fresh fallback으로 보존한다. 예상 밖 배열에서 중복 가능성은 데이터 유실보다 낫다는 fail-safe 비용으로 명시적으로 수용한다. exact-text/substring/참조 동일성은 어느 경로에서도 쓰지 않는다.
...
턴 전체에 고정된 `id`와 `answerPrefix` 대신 **소유자 없는 turn-level provisional preview 하나**, 어시스턴트 메시지 단위의 **순서 있는 lane 목록**, block-delivery credit, final-reconciliation 상태를 둔다. 현재 lane 하나만 보존하면 늦은 queued callback을 과거 소유자에게 돌려줄 수 없고, preview를 첫 lane에 미리 귀속하면 tool scaffold ghost를 지울 수 없다.
...
1. 첫 `onAssistantMessageStart`는 최초 lane을 가리키는 no-op이고, 이후 경계는 이전 lane을 `closed`로 표시한 뒤 새 current lane을 연다. lane ID는 경계에서 미리 확정하지 않고 그 lane이 실제 assistant text/block/final로 wire-visible해질 때 할당한다.
2. 경계 시점에 내용이 없는 앞 lane은 버블을 만들지 않지만 **즉시 폐기하지도 않는다.** queued callback이 뒤늦게 소유권/본문을 제공할 수 있으므로 `unresolved` predecessor로 남긴다. 뒤 lane의 partial/final은 메모리에 ingest하되 unresolved predecessor를 추월해 wire에 내보내지 않는다. 앞 lane이 materialize되거나 terminal callback drain에서 실제로 비어 있었음이 확정된 뒤 generation 순서로 푼다.
...
8. **`deliver(kind:"final")`은 payload 분류와 final-reconciliation state로 처리한다.**
   - 첫 final을 처리하기 전에 이미 enqueue된 callback/delivery 작업을 drain하고 late-owner epoch를 닫는다. 이 terminal barrier에서 본문·queued block·credit이 없는 closed predecessor를 `empty`로 확정해 preview claim/emission barrier를 푼다.
   ...
   - terminal error가 ordinary answer보다 먼저 오면 `retained-answer-replay` mode를 열고 `replayCursor`를 가장 이른 non-empty assistant lane으로 둔다. 이후 non-notice answer final은 generation 순서의 lane ID에 authoritative update로 재적용한다. 이미 materialize/settled된 A도 같은 ID로 다시 보내므로 `[error,A,B]`가 A를 복제하지 않는다.
   ...
   - replay cursor로 상관할 lane이 없거나 예상 밖의 추가 non-error final이 오면 내용을 비교해 버리지 않고 진단 후 fresh fallback ID로 보존한다. 이 계약 밖 배열에서는 중복 가능성을 명시적으로 수용하지만 데이터는 잃지 않는다.
   - draft lane이 없는 mode(block/off)는 기존 plain append 경로를 그대로 유지한다.
9. retained-answer replay는 **lane settle latch의 재진입이 아니다.** 같은 lane의 normal-finalize/cleanup은 `settleResult`로 한 번만 보내지만, core가 별도 delivery call로 준 authoritative replay는 기존 lane ID에 별도 update를 한 번 수행한다. 각 delivery 호출의 `visibleReplySent`는 그 호출이 실제 수행한 send 결과이고 turn 단위 `finalReplyDelivered`는 OR로 누적한다.
...
- block delivery credit 소비/fallback 및 final notice/replay/ordinary-answer reconciliation
- abort/error cleanup
...
ordinary final이 current lane의 terminal slot을 소비할 때 앞 메시지를 인용하거나 반복해도 boundary가 갈랐으면 별도 버블이다. 그 final이 current partial을 크게 재포맷해도 해당 lane만 교체한다. leading-error retained-answer replay는 텍스트가 같아서가 아니라 §5.4의 구조화된 phase와 generation cursor 때문에 기존 lane ID로 간다. 그 cursor로 상관할 수 없는 추가 final만 §6.2-8의 fresh fallback을 탄다.
...
- `streaming.mode:"progress"`: tool/item 줄만 provisional preview에 보인다. ordinary answer final이 오면 그 ID를 claim해 원자적으로 교체/정착한다. terminal notice와 retained-answer replay도 §6.2-8의 같은 reconciliation을 쓰며, 모든 추가 final을 무조건 fresh append하지 않는다. durable answer가 전혀 없는 clean-silent turn만 no-delete 제약 때문에 기존처럼 scaffold 자체를 settle한다.
...
6. abort/error cleanup은 이미 정착한 lane을 normal-finalize로 다시 보내지 않는다. 먼저 event queue를 drain하고 late indexless owner를 닫아 늦게 채워진 predecessor를 generation 순서로 정착한 뒤, current lane만 snapshot으로 방어 정착한다. 정착 조건은 **실제 assistant text/queued block 존재**다. `ProvisionalPreview.started`는 lane content의 증거가 아니며, preview가 durable lane에 claim되지 않은 tool-only turn에서만 legacy scaffold settle 조건으로 쓴다.
...
- 메시지 경계 close/rotate, preview ID claim/reuse, unresolved predecessor 보존, persistent late-indexless owner, generation-order emission barrier, queued block delivery credit/fallback, lane별 settle latch를 추가한다.
...
- first-final 직전에 queued callback 작업을 drain해 empty predecessor와 late-owner epoch를 닫는다. final payload는 terminal notice, retained-answer replay, ordinary current-lane answer, ambiguous fallback으로 분류한다.
- leading terminal error는 assistant lane을 소비하지 않고 replay cursor를 연다. 뒤의 retained A/B payload는 generation 순서의 기존 lane ID에 authoritative update하며, 예상 밖 unmatched payload만 fresh fallback으로 append한다.
- 현재의 “final 하나가 턴 전체 draft를 교체한다”는 주석과 분기를 제거한다.
- 앞 lane/replay send 실패를 격리하고 final delivery별 결과 및 턴 단위 OR 회계를 유지한다.
...
| M6f | B boundary/partial 뒤 A1/A2 `onBlockReplyQueued`가 모두 index 없이 늦게 도착 | A1 뒤에도 late owner 유지, A에 A1+A2 순서 보존, B 불변 |
...
| M11 | A settled/B partial 뒤 `[terminal error, A, B]` final 배열 | error는 lane 미소비, A/B는 같은 기존 ID에 replay되어 중복 없음 |
| M12 | replay cursor/current lane 어느 쪽에도 상관되지 않는 추가 non-error final | 진단 + fresh fallback으로 보존, 기존 lane 불변 |
...
| I9 | progress mode | provisional scaffold ID를 ordinary answer가 claim; notice/replay는 partial과 같은 reconciliation |
...
| I12 | partial 모드에서 A materialized/B partial 뒤 core final `[terminal error, A, B]` | assistant subsequence는 기존 두 ID의 A/B 정확히 한 번, error notice 한 번, outcome error |
...
| I16 | commentary-only A의 A1/A2 queued callback이 B boundary/partial 뒤 모두 index 없이 도착 | A에 A1+A2 모두 보존, B 별도 정착, 두 block delivery는 credit만 소비 |
...
| C7 | A/B 기존 ID + fresh error notice + 같은 A/B ID의 retained-final replay | A/B는 제자리 update, error 한 번, 중복 버블 없음 |
...
- terminal-error retained-answer fixture는 plugin integration(I12)에서 `[error,A,B]` exact wire sequence를 고정한다. provider별 error 재현에 기대지 않으므로 live e2e를 불안정하게 만들지 않는다.
...
| 첫 final 뒤 모든 payload를 fresh ID로 append | 고정 builder의 `[error,A,B]`에서 이미 materialize된 A까지 복제해 `[A,error,A,B]`를 만든다. leading-error replay cursor는 A/B의 기존 generation/ID를 재사용하고 unmatched payload만 fallback append한다. |
...
- [ ] ordinary answer final만 current lane의 terminal slot을 확정한다. terminal notice는 lane을 소비하지 않고, retained-answer replay는 기존 lane ID에 재적용되며, unmatched payload만 fresh fallback으로 보존한다.
- [ ] A materialized/B partial 뒤 `[error,A,B]`에서 assistant A/B는 정확히 한 번씩 남고 error notice도 한 번 보인다. answer → timeout/warning 순서에서도 어느 payload도 settle latch에 삼켜지지 않는다.
...
- [ ] history 화해 비대칭(C3/C4), later-snapshot adoption, 다중 draft watchdog(C5a/C5b), provisional-ID reuse(C6), retained-final same-ID replay(C7)가 테스트로 고정된다.
...
`packages/client/src/nats-client-wrapper.test.ts`에 §10의 C1~C7을 추가한다. 플러그인 변경과 완전히 독립이며 합성 프레임만으로 검증된다. 목표는 프로덕션 무변경이다. C6/C7은 새 프로토콜 없이 provisional ID reuse와 same-ID retained replay가 ghost/중복 없이 작동함을 고정한다.
...
**PR 2 — #94 본체 (대, 원자적)**
provisional preview + ordered/unresolved lane 모델 + inbound 배선 + M1~M12(세분 케이스 포함) / I1~I16 + e2e 게이트. **더 쪼개면 깨진다** — inbound가 경계/queued block/delivery를 넘기지 않으면 adapter는 preview claim, late ownership, final replay와 emission barrier를 운용할 수 없고, adapter에 retained lane/credit이 없으면 late callback 보존이 성립하지 않는다. §6.5 fail-safe도 못 뗀다. 현재 코드에 이미 `absorbedMissedBoundaries` 방어가 있어서, 빼고 먼저 내보내면 #23이 막아둔 것을 되돌리는 셈이다.
...
1. `packages/plugin/src/channel.test.ts`의 기존 “두 assistant 메시지가 한 ID에 합쳐진다” 테스트를 두 ID/두 버블 기대값으로 바꾸고, final이 앞 메시지를 인용하는 실패 테스트를 먼저 추가한다. 이어 tool scaffold → empty boundary → answer, late A1/A2(index 없음), `[error,A,B]` retained replay의 red test를 추가한다.
2. `packages/plugin/src/message-adapter.ts`의 턴 고정 `id`/`answerPrefix`를 provisional preview, ordered lane 목록, unresolved predecessor, persistent late-indexless owner, block-delivery credit, final replay cursor, lane별 settle latch로 교체한다.
...
- ordinary answer final만 current lane terminal slot을 소비한다. leading terminal error는 retained-answer replay cursor를 열고 A/B는 기존 lane ID에 재적용한다. notice와 unmatched fallback만 별도 ID를 쓴다.
...
- settle latch는 턴별이 아니라 lane별 normal terminal send용이다. preview의 `started`와 lane의 `started`를 분리하고, authoritative retained replay는 cached settle 결과에 삼키지 않는다.
