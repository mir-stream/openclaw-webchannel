...
- [x] build/typecheck/plugin tests/full tests가 모두 통과한다. — **EVIDENCE:** `cf69076` 외부-sandbox 실측으로 `npm run build` exit 0; `npm run typecheck` exit 0; `npx vitest run packages/plugin` 1794 passed/7 skipped/0 failed; `npm test` 144 files/2582 passed/0 failed.
