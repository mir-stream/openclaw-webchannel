...
| T3b | boot 레벨 (구현 중 재설계 — live 실측 결과 반영): OpenClaw **core** gateway는 자기 포트의 모든 경로에서 WS upgrade를 101로 수락하고 인증 제어 프로토콜 `connect.challenge`를 보낸다(코어 설계, plugin이 제거 불가). 따라서 "연결 거부" assert는 성립 불가능하고, 올바른 invariant는 **비구별성**: `/webchannel/ws`가 미등록 canary 경로와 동일 status + (101이면) 첫 프레임이 core challenge이며 webchannel 프로토콜 frame marker 부재 | `e2e` live harness probe (TechLead 최종 게이트에서 실행) |
