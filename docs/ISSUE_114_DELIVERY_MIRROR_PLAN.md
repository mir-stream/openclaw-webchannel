...
| **N6b** ⭐ | persist-before-publish를 "**보내기로 결정하기 전에도** 기록"으로 읽어, 거절된 send(transport down, fail-closed 세션키 없음)까지 저널링 | **"wire write 전"이지 "거절 전"이 아니다.** §16.2-2의 유령 논증은 **commit-vs-push 순서**(둘 사이의 크래시)에 관한 것이고, 시도조차 안 한 프레임은 다루지 않는다. 반증은 store가 아니라 **호출자**에 있다: 실패 + preview 사용 불가면 `reserveProvisional`이 매 시도마다 **새 `nextMessageId()`**를 발급하고 `lane.id ??=`는 성공에만 실행되며 `rollbackReservation`은 fresh id에 no-op, `lastProgressSentAt`도 성공에만 갱신돼 throttle이 실패 구간 내내 무력하다 ⇒ **`lane.id`·preview 둘 다 없는 구간**(⇒ fresh id, `rollbackReservation` no-op)에서 **재시도마다** `placement{X₁},{X₂},{X₃}…`를 낳고, id가 매번 달라 `journal_placement_once`가 못 접으며 #240 replay에서 전부 유령 빈 말풍선 = **N8을 gaining 방향으로, 무한정, 우리가 만들어서.** (성공 프레임이 하나라도 있으면 `lane.id` 고정 ⇒ 반복은 한 행으로 접힌다. 무한 사슬은 **첫 시도부터 실패하는 lane** 전용이고, fail-closed 세션키 없음이 정의상 그 상태라 **지배적**이다.) 거절을 안 적어도 잃는 건 없다(`false`는 이미 호출자가 delivery failure로 기록). 코드: `nats-channel.ts` `sendToPeer` — 훅은 세 거절 **아래**, wire write **바로 위** | "안전한 방향으로 실패하라"가 **컴포넌트만 보고** 판정 가능한 것처럼 느껴짐. 실제로는 **실패 후 호출자가 무엇을 하는가**에 대한 주장이고, 그건 남의 코드다 (2026-08-25 실제 발생: Advisor가 이 배치를 지시 → 리뷰 2라운드가 뒤집음) **예외 (#341, 2026-09-03):** `approval_request`/`approval_resolved`는 **core의 안정된 approval id**를 싣고(시도마다 재발급되는 id라는 이 행의 반증이 적용되지 않음), **승인 상태 자체가 push와 무관하게 서버에 존재**한다(`approvals.ts`의 pending/resolved 스토어 + register 시 `approval_snapshot` 재전달) → 기록 시점에 저널링한다. 규칙 자체(request 행이 있어야 resolution 행을 쓴다 — 배달 시점에, 아니면 resolve 시점에 먼저; 예외 = pending 레코드가 이미 사라진 경우)는 `approvals.ts` `updateEntry`의 "THE APPROVAL PAIR RULE" 한 곳에만 서술한다. ⚠️ 계정 맵은 **일시적**이다(재시작마다 delete/set) — "채널 없음"을 영구 상태로 읽어 resolve 쪽을 무조건 저널링한 것이 라운드 1의 결함이었다. ⚠️ **규칙 전문과 그 유일한 예외(pending 레코드가 이미 evict된 경우 → verdict만 저널링, reducer의 no-op이 fallback)는 `approvals.ts`의 `updateEntry`에 "THE APPROVAL PAIR RULE"로 한 번만 적혀 있다** — 여기 포함 다른 곳은 전부 그리로 가리킬 뿐, 다시 서술하지 않는다(라운드 2가 일곱 군데에 절대형으로 복제했다가 되돌린 실패). ⚠️ 대가는 N8-gaining 두 방향이며 둘 다 알고 받아들였다: (1) **내용** — pending 동안 어느 피어도 register하지 않은 카드(만료로 끝나든 다른 채널에서 resolve되든)는 history에 카드+결정으로 남지만 live에는 없었다(register했다면 `approval_snapshot`이 live로 재무장했고, 그 경우 catch-up 행은 history가 live를 따라잡는 것이다); (2) **순서** — resolve 시점에 뒤늦게 쓴 request 행은 그때 seq를 받으므로, 카드가 열려 있는 동안 도착한 메시지들 **아래**에 렌더된다(live는 register 시점 위치). 텔레그램 서버 의미론(서버는 만들었고 기록했다, 기기가 놓쳤다) 그대로다. 다른 모든 프레임 타입에는 이 행이 그대로 적용된다. |
...
- ~~ACK 직접 경로(control-lane, known-outcome 단락, cancelled debounce; `nats-account-runtime.ts:981-985,1135-1145`, `bounded-inbound-debouncer.ts:371-386`)가 durable 버블을 내면 각각 journal 훅 필요 — egress 콜백이 덮는지 구현 시 확인(대부분 egress 통과).~~ → **#239 half 3에서 답이 나왔다 — 다만 셋 중 둘은 "훅 불필요"가 아니라 갭 쪽으로:**
  - **known-outcome 처리 (#344):** `accepted` 마커만으로 접수를 확정하지 않는다. flush가 `randomId ?? wireId`로 저널을 조회하고, 행이 없으면 기존 마커를 같은 key gate 안에서 회수한 뒤 정상 접수 경로로 재진입한다. 새 receipt가 rollback을 소유하므로 뒤쪽 append가 실패해 앞쪽 행만 남아도 다음 replay가 턴을 재접수할 수 있다. 행이 있으면 기존 id·seq로 재-ack하고, `committed` 에코는 `random_id`가 있을 때만 붙인다. 저널 append 위치는 footer 한 곳이다.
    - **복구 범위는 marker→row다.** durable 순서는 여전히 마커 → 저널 행 → turn dispatch이며 ack은 저널 뒤에 나간다. 행이 쓰인 뒤 턴 시작 전 크래시는 marker+row로 남아 재-ack되므로 이 변경이 복구하지 않는다. 행 기록 후 dispatch 복구는 [#369](https://github.com/mir-stream/openclaw-webchannel/issues/369)의 별도 후속 범위이며, 마커 소실 후 중복 실행은 [#355](https://github.com/mir-stream/openclaw-webchannel/issues/355)에 있다.
    - **접수할 수 없는 리더는 고아 마커를 확정하지 않는다.** overflow resolver는 행 없는 `accepted`에 침묵하고, debouncer hot path는 refusal만 처리한다. 상세 읽기 계약은 `ingress-outcome.ts`의 `OutcomeLookup`에 있다.
    - **`/stop`은 `cancelled`로 억제한다.** 취소 마커를 먼저 영속화하고 약한 마커는 이후 lookup에서 정리한다. 실패하거나 rollback된 새 receipt 때문에 기존 억제까지 지워지지 않는다. replay는 재접수 없이 ack하며, 이미 저널 행이 있으면 id·seq 에코를 유지한다. 행이 없을 때의 취소 버블/history 정책은 [#292](https://github.com/mir-stream/openclaw-webchannel/issues/292)와 별개다.
    - **기존 상태의 한계:** 행 없는 마커(pre-journal 빌드, 없어진 저널)는 replay에서 재접수된다. 구버전이 `accepted`로 기록한 취소도 같은 상태여서, ack 유실 후 TTL 내 replay 시 취소한 턴이 실행될 수 있다. 이 구현은 두 과거 생산자를 구분하지 않는다.
  - **cancelled debounce = 훅 불필요가 아니라 control-lane과 같은 급의 실제 N8 갭이다(측정).** `bounded-inbound-debouncer.ts`의 `cancelKey`는 **inflight ∪ waiting**을 `exactUnion`으로 묶어 `onCancel`에 넘긴다 ⇒ 이미 `onFlush`가 처리 중인 메시지도 중간에 취소된다: `retainedItem.isActive()`가 뒤집혀 `invalidated` early return이 배치를 **저널 행 없이** 롤백하고, 이어서 `onCancel`(`nats-account-runtime.ts`)이 `result.write.commit()`으로 outcome `cancelled`(구버전은 `accepted`)를 적고 **ACK한다.** 그리고 그 메시지는 애초에 **클라가 publish한** 것이라(그래서 replay ledger에 있다) 클라엔 이미 user 버블이 그려져 있고 `/stop`은 그걸 지우지 않는다 — `markHeldRetracted`(`nats-client-wrapper.ts:1275-1291`)는 wire에 나간 적 없는 `this.held`만 잘라낸다. ⇒ **live엔 user 버블, 저널엔 행 없음 = N8 losing**, 바로 아래 control-lane과 같은 모양이다. "`/stop`이 **죽인** 텍스트라 durable 행이 있으면 안 된다"는 **오답, 폐기.** half 3 범위 밖인 이유도 control-lane과 같다: 그 브랜치엔 "접수 안 함"을 표현할 rollback 경로가 없고, 해소하려면 **메시지를 저널할 것이냐 버블을 회수할 것이냐**는 제품 결정이 필요하다. → **#292**.
...
2. **persist-before-publish** (codex5/claude9). id/seq 배정 + 이벤트 커밋을 **wire push 전에**(user·agent 공통). egress 시점엔 텍스트가 이미 있으니 유령 없음 = Telegram persist-then-deliver. → v5 §15.8의 commit-after 결정 **뒤집음**(안전 방향: history엔 있고 재접속이 따라잡음).
   ...
   - ⭐ **남는 창 = `sealEnvelope`/`transport.publish`가 커밋 **후** throw하는 경우 — 그리고 #347 이후 그건 창이 아니라 "성공한 send"다.** 우리가 Telegram **서버**니까, 메시지는 store가 id·seq로 담은 순간 존재하고 publish는 **push**다. push 실패는 send 실패가 아니라 클라의 다음 `get_difference`가 메우는 구멍이다(#244가 그 기계를 이미 깔았다: 저널된 프레임은 전부 seq를 소비하므로 못 받은 클라는 다음 seq-bearing 프레임에서 구멍을 보고 힐한다 — 단 자기 ack 에코까지 gap 판정에 넣는 #356 half A의 커서(PR #362)에서 그렇고, 그 전 클라의 advance-only ack은 그 구멍을 세션 동안 닫아 버렸다; 행 하나가 peer의 wire 예산을 넘는 #325 write side는 어떤 경로로도 못 나른다). 그래서 `sendToPeer`는 **행이 커밋됐으면 `true`** 를 돌려준다. 옛 문구 "`bubble`(id 안정)에서는 안전 방향이 맞다 **(단 보편적이지 않다, #278)**"는 **철회**한다 — 그 bubble이 lane의 **첫 성공처럼 보인 프레임**일 때 거짓이었고, 그게 정확히 그 문구가 커버한다고 주장한 모집단이다(#347 실측: `bubble:X=tA` + `seal[Y=tA]` ⇒ history 2×tA, live 1× = N8 gaining, 우리가 만든 것). **행이 없는 프레임**(비-durable 타입, id 없는 durable 프레임, 저널 write가 실패한 프레임)은 힐할 소스가 없으므로 여전히 `false`. **inbound 쪽 대응 창은 다르다**: 배치 부분 append 후 거절(#283) + 거절-후-재시도 순서 역전(#282), 둘 다 #240 소관.
...
⚠️ **텔레그램과 어긋나는 곳은 다섯이고, 다섯 다 근거가 있다.**
...
**서버 쪽**(`history-serve.ts`): `fitDifference`는 **행마다 한 번씩 재는 pass 하나 + 생존자 prefix에 대한 이분 탐색 하나**로 답변을 맞춘다. 단일 행 판정·prefix 탐색·발행은 **같은 응답 생성 함수**를 쓰며, 실제로 덮는 구간에 맞는 `partial`·`maxSeq`를 측정한다. 전체 high-water나 `partial: false`를 미리 가정하면 실제 부분 응답에는 들어가는 행까지 잘못 생략할 수 있다. 혼자서도 difference에 안 들어가는 행은 건너뛰어 catch-up을 진행한다(#343); live 프레임은 크기가 달라 skip만으로 live 미배달을 판단할 수 없다. 생존 행 뒤의 생략된 꼬리까지 덮으면 예산을 넘는 경우, 생존 행까지만 부분 응답으로 보내고 다음 요청에서 꼬리를 처리한다. 읽기는 다른 두 read처럼 **deferred**이며, 요청별 floor·nonce를 **FIFO 큐(대기 최대 8)**에 보관한다(#348). 실행 중인 read+publish는 피어당 하나이고, 큐가 넘치면 **가장 최신 대기 요청을 교체**한다. 밀려난 요청의 기기는 5초 타임아웃으로 재시도한다. 이 제한은 **동시성과 깊이**를 묶으며 rate 제한은 아니다.

**클라이언트가 서버를 믿지 않는 두 지점**(라운드 3): ① 응답이 **floor를 못 옮기면** 그건 settle이 아니라 **stall**이다 — catching-up과 재시도 예산을 유지하고, **요청이 걸어둔 타이머를 건드리지 않는다**(라운드 3은 여기서 타이머를 다시 걸었는데, 그건 매번 새 5초를 시작하는 것이라 5초보다 빨리 stall하는 peer가 마감을 영원히 밀어낸다 — 10분 시뮬레이션에서 요청 1개, 프레임 12 001개 보관, 화면 정지). 판정은 `maxSeq <= afterSeq`이면서 ⑴ `partial`이거나 ⑵ 이벤트 하나도 못 접고 throw했거나 ⑶ **버퍼에 프레임이 남아 있을 때**다 — ⑶이 없으면 같은 폭주가 `partial:false` 문으로 되돌아온다(빈 응답 50개 → 요청 51개). 버퍼가 빈 `partial:false` 빈 응답만이 **헛detection 해제**로 settle한다. settle하면 버퍼가 재투입되고 그 프레임이 gap을 다시 열어 **예산이 0으로 리셋된 채 RTT 속도로** 재요청한다(실측: 응답 50개 → 요청 51개). ② catching-up 중 **seq 없는 프레임도 버퍼에 담는다** — 즉시 적용하면 응답이 실어올 아래쪽 행들보다 위에 붙어 live≠history가 된다(`reasoningDurable` 기본 OFF ⇒ reasoning은 전부 seq 없음). 그리고 보관 프레임은 응답 이벤트와 **seq 순서로 병합**해서 접는다 — 도착 순서대로 응답 "뒤에" 재투입하면 리듀서가 last-write-wins라 응답이 막 세운 상태를 낡은 프레임이 덮는다(#343으로 서버가 건너뛴 행 자리가 바로 그 모양이다): tool의 낡은 `start`가 배달된 `end`를 덮고, 보관 중이던 `approval_request`가 이미 해소된 승인의 **누를 수 있는 카드**로 다시 붙고, `seal`이 지운 답변이 되살아난다. seq 없는 프레임만 그 순서에 자리가 없어 뒤에 도착 순으로 붙는다. 재투입 때 무엇이 대체됐는지는 **한 갈래**로 판단한다: 자기 본문을 갖지 않는 프레임(seq 없는 프레임, 그리고 `progress` — 그 행 `placement`는 본문을 싣지 않는다)은 **응답이 그 id의 본문을 썼을 때만** 버리고, 나머지는 **응답이 그 seq를 접었을 때만** 버린다. id로 버리는 걸 seq 있는 프레임까지 넓히면 응답이 커버한 범위 **위쪽까지** 버리게 된다 — tool 행은 dedupe되지 않아 phase마다 한 행이므로, `tool{start}@12`를 실은 슬라이스가 보관 중인 `tool_activity{end}@20`을 버리고 다음 슬라이스가 20을 skip하면 카드가 세션 내내 "running"으로 남는다.

**#349도 이 PR에 포함**(사용자 승인): replay의 `placement`는 live `working` 상태를 만들지 않는다. 빈 자리는 기존 reducer의 내부 순서에 유지하고 공개 snapshot에서는 숨긴다. live progress나 확정 본문이 오면 같은 자리를 공개하며, 이미 활성인 live draft의 텍스트·hold는 유지한다. 실제 `turn_settled`와 명시적 `/stop`은 이미 보관된 해당 progress를 종료한다.

**별도 후속**: #342 영속 커서 기반 재접속 복구, 일반 tier-1 history 갱신과 history 창 순서 정합성. #329의 일반 tombstone 보존도 별도다.
