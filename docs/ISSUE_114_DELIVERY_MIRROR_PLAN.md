...
- **"메시지"는 전부 durable, "표시기"만 ephemeral** (사용자 정정 2026-08-23, 북극성 직결):
  ...
  - 🟢 **reasoning: #242 half 2에서 클라까지 착지 (2026-08-27). half 1의 "서버 쪽만" 표기는 이 줄 아래 ✅ 항목들로 갱신됐다 — 남은 갭 두 개(#304, 순서)는 여전히 열려 있으니 "전부 닫혔다"로도 읽지 말 것.**
    ...
    - ⚠️ **"tool_activity·approval_\*은 여전히 non-durable"은 이제 approval에만 참이다 (half 3, 2026-08-28).** 갈라진 지점을 그대로 기록한다:
      - **`tool_activity` = durable (half 3)** — 프레임을 **verbatim delta로 저널**하고 병합은 공유 reducer의 `applyTool` 하나만 한다(seam에 accumulator를 두면 plugin이 계산한 projection이 되어 N8). reasoning의 `final` 게이트와 **반대 방향**이고 이유는 프레임 모양이다: 닫는 프레임에 `name`·`argKeys`가 없어서 `final` 하나만 저널하면 **결정론적 partial**이 남는다. ⚠️ **별도 durability opt-in이 없다** — 게이트는 live lane 자체(`streaming.mode ∈ {progress, partial}`)뿐이고 플러그인 기본값은 `off`다.
      - **`approval_request`/`approval_resolved` → half 4.** "half 3의 연장"이 아니라 **양방향 메시지**라서다: 클라의 `approval_decision`이 돌아와 내용이 바뀌므로 **#241**의 typed edit/revision 모델이 먼저 있어야 한다.
      - **`approval_snapshot` → 영구히 `null`.** 이건 메시지가 아니라 **replay**다(§15.9의 message-vs-indicator). "아직 안 했다"가 아니라 "할 일이 아니다".
      - ⚠️ **half 3이 새로 연 갭 하나**: tool의 커서 공간에 **run-scoped id**가 들어왔다. `createAgentToolActivitySink`가 턴마다 새로 만들어져 `tool-activity-1`이 매 턴 다시 나오는데, 페이징 커서는 **id 하나만** 싣는다 → 같은 id가 두 번 있으면 `findIndex`가 **옛 쪽**에 앵커해 그 사이 행들이 조용히 사라진다. 지금은 **ambiguous ⇒ 빈 페이지**(miss와 같은 정직한 정지)로 막아 뒀고, 진짜 수정은 `(turnId, id)` 복합 커서 — **#320**. 가드는 tool 전용이 아니라 **중복 id 전부**에 걸린다(피어가 되보낸 user id, #293).
...
5. **durable content = tagged union**(not {role,text}) (codex12/claude11-12, 사용자 tool-durable 지시). user·answer·notice·**tool·approval**·(content로 남길 reasoning) = stable id + 구조화 내용 + seq. 순수 표시기만 ephemeral. → DurableMessage 타입 확장.
   ...
   - ⚠️ **이 줄의 "넷 다 저널링되지 않는다"는 half 3이 갈랐다 (2026-08-28).** 원래 문장은 "`tool_activity`·`approval_request`·`approval_resolved`·`approval_snapshot`은 여전히 저널링되지 않으며 **#242 half 3** 소관"이었다(그전엔 "half 2"였다 — #242가 셋으로 쪼개지기 전의 forward reference). 현재:
     - **`tool_activity`는 durable이다 (half 3).** `DurableEvent`와 `DurableMessage` 양쪽에 `kind:"tool"` 멤버가 생겼다. ⚠️ **멤버 수는 두 union이 다르니 하나로 뭉뚱그리지 마라**: `DurableMessage`는 `text`/`reasoning`/`tool`로 **셋**(위 🟡 항목의 "아직 둘"은 half 2 시점 서술이다), `DurableEvent`는 `user`/`placement`/`bubble`/`seal`/`reasoning`/`tool`로 **여섯**이다(`durable-view-reducer.ts:501`의 "BOUNDARY 2: six kinds"). 저장 단위는 **프레임(delta) verbatim**이고 병합은 `applyTool` 하나뿐 — reasoning의 `final` 게이트와 반대이며, 이유는 닫는 프레임에 `name`·`argKeys`가 없다는 **실측된 프레임 모양**이다. tool 변종엔 reasoning처럼 `role`이 없고, `turnId`는 **필수**다(뷰가 `(turnId, id)` 쌍으로 주소를 매긴다).
     - **`approval_request`/`approval_resolved`는 half 4**(양방향 → #241 revision 모델 선행), **`approval_snapshot`은 영구히 `null`**(메시지가 아니라 replay). 자세한 근거는 §15.9의 half 2 항목에 있다.
     - ⚠️ **reasoning의 "서버 쪽만"도 옛말이다** — half 2가 wire row를 union으로 넓히고 projection의 drop을 없애고 클라 렌더를 reducer로 옮겼다. 남은 갭은 두 개(#304, 순서)뿐이고 §15.9의 reasoning 항목에 있다.
     - ⚠️ **tool이 물려받은/새로 만든 갭:** #304의 거절 창은 tool에선 "행 없음"이 아니라 **부분 병합된 잘못된 행**으로 나타나고(프레임 하나가 빠지면 이름 없는 완료 또는 영원히 안 끝나는 호출), 커서 공간엔 run-scoped id가 들어왔다(**#320**). 둘 다 `journal-history.ts` 헤더의 GAP 블록에 서술돼 있다.
...
| 모든 message/service 타입 durable vs tool/reasoning/notice/approval ephemeral | [#114] | 🟡 **부분 (#242 half 1 → half 3, 2026-08-28)** — **reasoning + tool**. 둘 다 `history` wire row·클라 `ChatMessage`·reducer가 tagged union이라 **리로드하면 보인다**. 게이트는 서로 다르다: reasoning은 `capabilities.reasoningDurable` 옵트인(기본 OFF), **tool은 별도 durability opt-in이 없고** live lane(`streaming.mode ∈ {progress, partial}`, 플러그인 기본값 `off`)만 탄다. 남은 갭 = **approval**(half 4; `approval_snapshot`은 replay라 영구 비대상) + 물려받은 **#304**(reasoning은 행 없음 / tool은 **부분 병합된 잘못된 행**)·**순서(GAP 2b)**·**#286**(quadratic replay)·**#311**(row-bounded page), 그리고 half 3이 연 것 둘 — **#320**(run-scoped tool id가 커서 공간에 들어옴)과 **#321**(`argKeys`의 키 개수·길이를 아무 데서도 안 막아 무제한으로 디스크에 닿음). 전체 서술은 §15.9 |
...
**우리 쪽 위험은 core가 아니라 2번이 깨지는 지점에 있다.** 순서-대응은 **우리 레인 시퀀스가 final 시퀀스와 보조를 맞출 때만** 성립한다. 레인 하나를 목록에서 떨어뜨리면 그 순간 어긋난다 — 현재 `flushBufferedOrdinaryFinals`가 후보를 `materializedAnswerLanes()`(전송 성공한 레인만)로 좁히는 것이 정확히 그 결함이다. 스트리밍은 했지만 전송이 실패한 레인이 빠지면서 인덱스가 밀리고, 그게 M173e의 오염이자 M212a가 유일본 텍스트를 삭제하는 원인이다. **고칠 것은 core에서 못 받는 정보가 아니라 우리가 스스로 버린 순서다.**
