...
- ~~ACK 직접 경로(control-lane, known-outcome 단락, cancelled debounce; `nats-account-runtime.ts:981-985,1135-1145`, `bounded-inbound-debouncer.ts:371-386`)가 durable 버블을 내면 각각 journal 훅 필요 — egress 콜백이 덮는지 구현 시 확인(대부분 egress 통과).~~ → **#239 half 3에서 답이 나왔다 — 다만 셋 중 둘은 "훅 불필요"가 아니라 갭 쪽으로:**
  - **known-outcome 단락 = 훅 불필요 — 단 이유는 좁은 쪽이다.** 새 텍스트의 **fresh admission이 아니라서** 이 seam이 그 행의 저자가 아니다 — 그게 이유의 전부다. ~~"재-append는 어차피 `journal_user_once` no-op"~~도 **같이 폐기**한다: 그 no-op은 **행이 이미 있을 때만** 성립하는데, 바로 아래가 행이 없는 경우다. (즉 #292가 훅을 원한다면 그 훅은 inert가 아니다.) ~~"이미 저널된 메시지의 replay이므로"~~는 **틀린 이유이며 폐기**한다 — 아래 cancelled-debounce 경로와 half 3 자신의 `non-string-text` 경로가 **저널 행 없이** `accepted`를 기록하므로 **`outcome=accepted`는 저널 행의 존재를 함의하지 않는다.** 그런 id의 replay는 `existing.status === "found"` 브랜치로 들어가 행 없는 outcome 아래에서 계속 재-ack된다. → **#292**.
    - ⚠️ **부분 갱신 (#344, 2026-09-02):** "훅 불필요"는 유지되지만(이 seam의 저널 호출부는 여전히 footer 하나뿐), **"행 없는 outcome 아래에서 계속 재-ack된다"는 더 이상 사실이 아니다.** `ingress-dedupe.ts`의 found/accepted 브랜치가 이제 저널을 **권위로 조회**한다: item이 `random_id`를 갖고 이 seam에 저널이 있는데 그 `random_id`의 행이 없으면 마커는 접수의 증거가 아니므로 **재-ack 대신 fresh-accept 경로로 흘려보내 저널·디스패치·`committed` 에코까지 정상 접수**한다(별도 훅이 아니라 fall-through라 호출부는 그대로 하나). 이로써 **record→턴-시작 크래시 창은 클라의 다음 replay에서 스스로 낫는다.**
      - **순서는 그대로 둔다.** §15.7의 "저널-먼저 → 확정"은 **wire** 순서로는 참이고(ack은 append 뒤에 나간다), **store** 순서로는 거짓이다 — `outcomeStore.record`가 SDK store에 마커를 먼저 쓴다. #344 전까지 이 문장이 둘을 뭉개고 있었고, 그 혼동이 이 버그가 살던 자리다. 뒤집으면 **행-있음/마커-없음** 창이 열리고 그쪽은 같은 텍스트에 답변 2개(#355의 마커-에빙션 항목)이다. 바뀐 건 **어긋났을 때 저널이 이긴다**는 읽기 규칙뿐.
      - **대가(문서화):** 행 없는 마커(pre-journal 빌드가 남긴 것, 재생성된 저널 파일)는 replay 시 그 턴을 **한 번** 다시 돌린다 — 마커 store의 7일 TTL과 클라 ledger로 유계.
      - ⭐ **그리고 그 조회는 `accepted`에게만 던진다 — `cancelled`는 별도 terminal outcome이다(#344 라운드 2).** "마커 있고 행 없음"은 크래시 창만의 상태가 **아니었다**: `/stop` 억제 3인방(`nats-account-runtime.ts` `onCancel`, `ingress-dedupe.ts` cancelled-inbound fallback, `inbound-overflow-resolver.ts` `recoverCancelled`)이 **의도적으로 행 없이** 마커만 적는다. 그게 `accepted`를 빌려 쓰는 동안 두 상태는 바이트 단위로 같았고, ack이 유실된(`sendAck` false, 또는 피어 오프라인 + at-most-once `.out`) 취소 메시지가 재연결 replay에서 **재접수되어 `/stop`이 취소한 턴이 다시 돌았다** — 라운드 1 리뷰가 실측한 결함. 이제 셋 다 `"cancelled"`를 적고(`ingress-outcome.ts`, 전용 네임스페이스 + 우선순위 최상위), found 브랜치는 그걸 **ack 후 폐기**한다(재접수 없음, `inbound_rejected` 아님, `committed` 에코 없음). 저널은 "접수됐나"에 답하고, outcome은 "애초에 돌 예정이었나"에 답한다 — 둘째 질문은 저널이 답할 수 없다.
      - **마이그레이션(좁지만 실재):** 이 변경 **이전** 빌드가 `accepted`로 적어둔 취소 마커는 TTL 7일 동안 크래시-창 마커와 구별 불가다. 그 메시지의 ack까지 유실됐다면 다음 replay에서 그 턴이 **한 번** 다시 돈다. (구버전 취소 × ack 유실 × TTL 내 replay) 세 조건이 모두 필요하고, 사후 복구는 불가능하다(두 경우가 옛 마커에서 동일하므로).
      - ⭐ **규칙은 하나다 — READER RULE(`ingress-outcome.ts`의 `OutcomeLookup`에 단 한 번 서술).** `accepted`는 **판정이 아니다**: 그 접수가 살아남았는지는 저널만 아는 사실이고, 리더를 가르는 것은 저널 접근이 아니라 **접수 능력**이다: 메시지를 접수할 수 있는 리더는 flush 경로의 found 브랜치 **하나뿐**이므로 행 없는 `accepted`에 답할 수 있는 것도 그곳뿐이다(행 있음 ⇒ 에코와 함께 재-ack, 행 없음 ⇒ **재접수**); 접수할 수 없는 리더(overflow resolver — 저널은 든다 —, debouncer fast path)는 행이 있을 때만 재-ack하고 없으면 침묵한다. 나머지 리더는 전부 "내가 결정할 게 아니다"로 넘긴다. 두 refusal(`overloaded`/`cancelled`)은 반대로 **어디서든** 판정 가능하다 — 저널이 뒤집을 수 있는 사실이 없으므로. **명단(census)이 아니라 규칙을 적는 이유:** 이 슬라이스에서 "이 결과를 읽는 리더는 A와 B뿐"이라는 문장이 **두 번** 나왔고 **두 번 다** 같은 변경 안에 빠뜨린 리더가 있었다(라운드 2는 overflow resolver, 라운드 3은 debouncer fast path — 심지어 그 리더를 찾아내는 grep을 함께 인쇄했다).
      - **답을 안 하는 것만으로는 replay가 보존되지 않는다(라운드 4에서 실측).** `lookupUnlocked`의 found 경로가 **hot 캐시를 덥히고**(`write.commit()`도 마찬가지), debouncer의 `peekOutcome` 단축이 retention 과금 **이전에** 그 캐시를 읽는다. 그래서 resolver가 방금 답하기를 거부한 그 고아 마커가 fast path에서 **ack되어 사라졌다** — `onFlush`·저널·found 브랜치에 영영 도달하지 못한 채. 이제 단축은 refusal에서만 발동하고, `onKnownOutcome`/`known-outcome` 결과 타입이 `IngressRefusal`이라 `accepted` 판정은 **타입상 표현 불가**다.
      - ⭐ **문(door)은 둘이고, 라운드 2는 하나만 고쳤다(라운드 3에서 발견).** `accepted` 마커를 읽는 두 번째 지점이 `inbound-overflow-resolver.ts`다 — raw 프레임을 보존하지 못한 id가 가는 경로. 거기서 마커를 종결 accept로 읽고 **ACK**했으므로 클라 ledger가 비워지고 그 메시지는 **영구 유실**됐다(seam 쪽 버그보다 나쁘다: seam은 최소한 다음 replay가 남았다). 크래시 없이도 도달한다 — §15.6 파괴적 컷오버가 저널 파일을 새로 만들면 살아있는 마커가 **전부** 고아가 되고, `DEFAULT_BUSY_TURN_LIMITS`가 세션당 32건이라 오프라인 백로그 재연결이 33번째부터 overflow로 들어간다. 이제 resolver도 같은 권위를 적용하되 **아무것도 발행하지 않는다**: resolver는 판정만 낼 수 있고 접수는 할 수 없으므로, 침묵이 ledger 엔트리를 살려 두고 그 메시지는 평소 flush 경로로 돌아와 저널·디스패치·에코된다.
      - **조회 키는 `randomId ?? wireId`다.** 라운드 2는 `random_id`만 넘겨서 구클라(토큰 미전송)의 존재 질문에 답할 수 없었고, 그걸 "범위 선택"이라 적었다. 실제로는 그 존재 검사 자체가 구클라 크래시-창 메시지의 **유일한 수리 수단**이었다. `appendInboundUser`가 `idempotency_key = randomId ?? turnId`로 적고 lookup이 같은 컬럼을 읽으므로 인자 하나만 넓히면 된다. 구클라도 동일하게 낫고, 다만 `committed` 에코는 `random_id`로 키잉되므로 **bare ack**을 받는다.
      - **남는 #292:** id-less admit 경로(#293), 그리고 `non-string-text`가 행 없이 `accepted`를 적는 **생산 지점**. #344는 그 지점들을 고치지 않고 **replay가 스스로 나을 수 있게** 만들 뿐이다.
