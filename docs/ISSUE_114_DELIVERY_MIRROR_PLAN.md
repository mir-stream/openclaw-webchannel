...
- **"메시지"는 전부 durable, "표시기"만 ephemeral** (사용자 정정 2026-08-23, 북극성 직결):
  ...
  - 🟡 **reasoning: #242 half 1에서 서버 쪽만 착지 (2026-08-27). 남은 갭을 아래에 명시한다 — 닫혔다고 읽지 말 것.**
    - **착지한 것:** ① wire `reasoning` 프레임에 **`final?: boolean`** 추가(`channel-contract.ts`); ② `createReasoningDraftController`가 burst가 닫히는 **네 지점**(`endBurst`, `pushDurableBlock`의 두 갈래, **`stop()`**)에서 `final: true` 프레임을 **정확히 하나** 낸다; ③ `journalEventForOutbound`가 **그 프레임만** 저널링 → **burst당 1행**; ④ 공유 reducer에 `kind:"reasoning"` 이벤트 + `applyReasoning`(id upsert, 없으면 tail append = **배달된 자리**를 잡는 슬롯 클레임); ⑤ `DurableMessage`가 **tagged union**이 됨(`text` | `reasoning`, §16.2-5의 첫 조각); ⑥ `journal-history.ts`가 reasoning 행을 접어 view에 넣는다.
