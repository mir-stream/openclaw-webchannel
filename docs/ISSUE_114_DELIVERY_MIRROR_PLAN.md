...
**클라이언트가 서버를 믿지 않는 두 지점**(라운드 3): ① 응답이 **floor를 못 옮기면**(`partial`인데 `maxSeq <= afterSeq`, 또는 이벤트 하나도 못 접고 throw) 그건 settle이 아니라 **stall**이다 — catching-up을 유지하고 재시도 예산을 그대로 쓰며 타이머 주기로만 재요청한다. settle하면 버퍼가 재투입되고 그 프레임이 gap을 다시 열어 **예산이 0으로 리셋된 채 RTT 속도로** 재요청한다(실측: 응답 50개 → 요청 51개). ② catching-up 중 **seq 없는 프레임도 버퍼에 담는다** — 즉시 적용하면 응답이 실어올 아래쪽 행들보다 위에 붙어 live≠history가 된다(`reasoningDurable` 기본 OFF ⇒ reasoning은 전부 seq 없음). 대신 재투입 때 **응답이 그 id의 durable 본문을 썼으면** 버린다(초안이 최종본을 덮어쓰지 않게). `progress`만은 seq가 겹쳐도 **버리지 않는다** — 그 행(`placement`)은 본문을 싣지 않으므로 "이미 배달됨"이 슬롯엔 참이고 텍스트엔 거짓이다.
