...
**상태:** 후보 ③(**플러그인이 게이트웨이 포트로 서빙**, `src/static-assets.ts` + `/webchannel/` 라우트)을 **구현 완료**(PLAN.md Phase 3). dev는 Vite(프록시), 배포는 **client 데모** `npm run build:demo`(→`packages/client/dist-demo`, base `/webchannel/`) 산출물을 게이트웨이가 서빙. (구 React 위젯 서빙은 위젯 삭제로 대체됨.)
...
**SaaS 임베드 경로**
- **크로스오리진 게이트웨이 URL 옵션** — `openclaw-webchannel-client`의 `url` 옵션으로 **해결됨**(다른 오리진 게이트웨이 직접 지정).
- **SaaS 실연동** — 백엔드가 서버측 `issueWebChannelTicket`로 발급 → 클라이언트 `getTicket`. (현재 데모는 브라우저 발급 = DEV 전용)
- `openclaw-webchannel-ticket` zero-dep 패키지 분리(SaaS 백엔드가 SDK 없이 소비할 때).
