...
1. 현재 `handleInboundMessage`가 await 중인 agent run의 exact peer lease
...
export class ApprovalOriginLeaseRegistry {
  ...
  resolve(input: { accountId: string; sessionKey: string }): ApprovalOriginLeaseResolution;
  clear(): void;
}
```
...
- 같은 handle의 `activate()` 반복은 no-op이다. `onAgentRunStart`가 같은 turn에 반복되어도 claim은 하나다.
- resolve는 active lease의 **distinct exact peer ID**가 0개면 `no_match`, 1개면 `resolved`, 2개 이상이면 `ambiguous`다. 같은 peer의 여러 lease는 계속 `resolved`다.
...
이 callback은 실제 agent run이 시작되어 tool approval을 낼 수 있는 시점을 나타낸다. pre-run denial/setup failure는 claim을 만들지 않는다. `controlLane === true`인 `/stop` turn은 activate하지 않는다.
...
1. non-empty `request.sessionKey`를 확인한다.
2. registry를 exact `(accountId, sessionKey)`로 resolve한다. `no_match`/`ambiguous`면 `null`.
...
- **reload/restart availability:** in-flight approval이 reload 뒤 replay되어도 lease가 없으므로 의도적으로 전달하지 않는다. durable cross-restart origin 증명은 별도 설계가 필요하다.
...
- [ ] registry의 immutable-handle, duplicate/ambiguity/exact-account/release/epoch-clear 불변식이 green이다.
...
- [ ] config reassignment, stored overwrite, account case collision, reload 뒤 wrong-peer publish가 0건이다.
