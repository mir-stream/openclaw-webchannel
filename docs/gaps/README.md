...
| **P1-3** reasoning lane | ✅ **built**: core live/durable reasoning → dedicated turn-correlated wire/state → collapsed sanitized UI; gated on the channel-private `capabilities.reasoning`, default ON (#113; absent → on, any present non-`true` value or malformed container → off), with persisted explicit session `/reasoning off` retained as a privacy veto; passes `streamReasoningInNonStreamModes` plus `reasoningPayloadsEnabled`; warns once per account per process when a successfully-answered turn's open lane got no frames | `ReasoningDraftController`; `resolveReasoningEnabled` (`account-config.ts`); `reasoning-opt-out.ts`; `reasoning`/`turn_settled`; `presentation.ts` |
