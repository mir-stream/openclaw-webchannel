...
| **P1-3** reasoning lane | ✅ **built**: core live/durable reasoning → dedicated turn-correlated wire/state → collapsed sanitized UI; gated on the channel-private `capabilities.reasoning`, default ON (#113; absent → on, any present non-`true` value or malformed container → off), with persisted explicit session `/reasoning off` retained through a raw-snapshot fail-closed privacy veto; passes `streamReasoningInNonStreamModes` plus `reasoningPayloadsEnabled`; complete durable blocks preserve full text under distinct ids, except the pinned CLI's exact replay of its still-open final live snapshot; warns once per account per process when a successfully-answered turn's open lane got no frames | `ReasoningDraftController`; `resolveReasoningEnabled` (`account-config.ts`); `reasoning-opt-out.ts`; `reasoning`/`turn_settled`; `presentation.ts` |
...
| **P1-8** turn control | ✅ **built**: `/stop` control lane (#25, `control-lane.ts`) + debounce/coalesce (#29, `inbound-queue.ts`) | control lane `index-nats.ts:724-822`; Stop button `composerButtonMode`/`composerInFlight` (`presentation.ts`), `applyComposerMode` (`widget.ts`) |
