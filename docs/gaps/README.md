...
| **P1-3** reasoning lane | ✅ **built**: native callback → dedicated turn-correlated wire/state → collapsed sanitized UI; gated on the channel-private `capabilities.reasoning`, default ON (#113; absent → on, any present non-`true` value → off), passing `streamReasoningInNonStreamModes` so core emits; warns once per account per process when a successfully-answered turn's open lane got no frames | `ReasoningDraftController`; `resolveReasoningEnabled` (`account-config.ts`); `reasoning`/`turn_settled`; `presentation.ts` |
