...
| `__openclaw.id` / 합성 id 의존 | #128 후속 처리 | 공개 SDK는 identity/order key 없이 `messages: unknown[]`만 제공한다. 같은 비공개 봉투의 `seq`를 대안으로 채택하지 않고, `.id` shape drift와 current/legacy 합성 cursor miss를 명시적으로 진단한다. |
