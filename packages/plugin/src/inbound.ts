 ...
 * final reconciliation. The error and leading-terminal-error predicates are
 * retained as defence-in-depth at this seam so every identity-less final after
 * either condition remains independent even if controller state changes later.
 * Ordinary answer finals go to `draft.finalize`, which owns the lane routing
 * (collapse-aware order) and falls back to an independent bubble on overflow.
 */
export async function deliverDraftFinalPayload(
  ...
  // #173: EVERY ordinary (non-notice, non-error) final now goes to `draft.finalize`,
  // which routes it to the lane it belongs to (collapse-aware order routing). The
  // controller emits an independent bubble itself only on overflow. Notices,
  // errors and any final after a leading terminal error still take the
  // independent/error route unchanged.
  const independent =
