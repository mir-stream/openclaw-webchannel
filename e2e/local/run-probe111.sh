...
PKG_BAK=/tmp/oc-probe111-e2e.pkgbak.json
...
cleanup() {
  ...
  if [ -f "$PKG_BAK" ]; then
    cp "$PKG_BAK" "$PKG_JSON"
    rm -f "$PKG_BAK"
    echo "[run-probe111] restored $PKG_JSON"
  fi
}
trap cleanup EXIT
...
ISSUER_PID=$!
echo "[run-probe111] enrollment-server pid=$ISSUER_PID — waiting for JWKS + NATS config…"
for i in $(seq 1 120); do
  if curl -fsS "http://127.0.0.1:$ISSUER_PORT/.well-known/jwks.json" >/dev/null 2>&1 \
     && [ -f "$OCH/operator.jwt" ] && [ -f "$OCH/resolver.json" ]; then
    echo "[run-probe111] issuer ready (JWKS up, NATS config written)"
    break
  fi
  if ! kill -0 "$ISSUER_PID" 2>/dev/null; then
    echo "[run-probe111] enrollment-server died early — log:"; cat "$OCH/issuer.log"; exit 2
  fi
  sleep 0.25
  if [ "$i" -eq 120 ]; then
    echo "[run-probe111] TIMEOUT waiting for issuer — log:"; cat "$OCH/issuer.log"; exit 2
  fi
done
...
NATS_PID=$!
echo "[run-probe111] nats-server pid=$NATS_PID (JWT-auth, ws://127.0.0.1:$NATS_WS) — waiting…"
for i in $(seq 1 120); do
  if grep -q "Server is ready" "$OCH/nats.log" 2>/dev/null; then
    echo "[run-probe111] nats-server ready"
    break
  fi
  if ! kill -0 "$NATS_PID" 2>/dev/null; then
    echo "[run-probe111] nats-server died early — log:"; cat "$OCH/nats.log"; exit 2
  fi
  sleep 0.25
  if [ "$i" -eq 120 ]; then
    echo "[run-probe111] TIMEOUT waiting for nats-server — log:"; cat "$OCH/nats.log"; exit 2
  fi
done
...
ECHO_PID=$!
echo "[run-probe111] echo server pid=$ECHO_PID (multi-msg marker=$ECHO_MULTI_MSG_MARKER, tool=$ECHO_MULTI_MSG_TOOL)"
...
# ---------------------------------------------------------------------------
echo "[run-probe111] building plugin dist/ from the working tree…"
( cd "$REPO" && npm run build --workspace=packages/plugin ) >"$OCH/plugin-build.log" 2>&1 || {
  echo "[run-probe111] plugin build FAILED — log:"; cat "$OCH/plugin-build.log"; exit 2
}
...
if [ -f "$PKG_BAK" ]; then
  echo "[run-probe111] stale $PKG_BAK found — restoring original package.json before re-swapping"
  cp "$PKG_BAK" "$PKG_JSON"
  rm -f "$PKG_BAK"
fi
cp "$PKG_JSON" "$PKG_BAK"
...
' "$PKG_JSON"
echo "[run-probe111] set plugin extensions → ./index-nats.ts"
...
JSON
echo "[run-probe111] wrote $OCH/.openclaw/openclaw.json (streaming.mode=partial)"
...
# ---------------------------------------------------------------------------
echo "[run-probe111] channels add (config-time device-flow enroll)…"
...
ADD_PID=$!
echo "[run-probe111] channels add pid=$ADD_PID — waiting for enrollment user_code…"
...
for i in $(seq 1 240); do
  ...
  if ! kill -0 "$ADD_PID" 2>/dev/null; then
    echo "[run-probe111] channels add exited before user_code — log:"; cat "$OCH/channels-add.log"; exit 2
  fi
  sleep 0.25
done
[ -z "$USER_CODE" ] && { echo "[run-probe111] TIMEOUT waiting for user_code — channels-add log:"; cat "$OCH/channels-add.log"; exit 2; }
echo "[run-probe111] enrollment user_code=$USER_CODE — approving…"
...
echo "[run-probe111] approve response: $APPROVE"
...
ADD_PID=""
if [ "$ADD_RC" -ne 0 ]; then
  echo "[run-probe111] channels add failed (rc=$ADD_RC) — log:"; cat "$OCH/channels-add.log"; exit 2
fi
CRED_FILE="$(node --import tsx "$REPO/scripts/resolve-storage-path.ts" \
  credentials "$TENANT" "$ACCOUNT_ID" "$OCH")"
[ -f "$CRED_FILE" ] || { echo "[run-probe111] creds NOT persisted at $CRED_FILE — log:"; cat "$OCH/channels-add.log"; exit 2; }
echo "[run-probe111] ✓ credentials persisted at $CRED_FILE"
...
echo "[run-probe111] ✓ re-asserted admission=register-hop + dmSecurity=allowlist + streaming.mode=partial for account $ACCOUNT_ID"
...
HOME="$OCH" OPENCLAW_HOME="$OCH" OPENCLAW_DISABLE_BONJOUR=1 \
  ...
  >"$OCH/gateway.log" 2>&1 &
GW_PID=$!
echo "[run-probe111] gateway pid=$GW_PID — waiting for structured account readiness (consume persisted creds)…"
for i in $(seq 1 240); do
  ...
  if printf '%s\n' "$LATEST_AGGREGATE" | grep -Eq "event=webchannel\.account_aggregate generation=[^ ]+ state=complete servingCount=1 totalCount=1"; then
    echo "[run-probe111] gateway ready (consumed creds + connected)"
    break
  fi
  if ! kill -0 "$GW_PID" 2>/dev/null; then
    echo "[run-probe111] gateway died early — log:"; cat "$OCH/gateway.log"; exit 2
  fi
  sleep 0.5
  if [ "$i" -eq 240 ]; then
    echo "[run-probe111] TIMEOUT waiting for structured account readiness — log:"; cat "$OCH/gateway.log"; exit 2
  fi
done
...
# ---------------------------------------------------------------------------
echo "[run-probe111] driving the multi-assistant-message turn…"
set +e
WEBCHANNEL_GW_URL="http://127.0.0.1:$GW_PORT" \
WEBCHANNEL_NATS_URL="ws://127.0.0.1:$NATS_WS" \
