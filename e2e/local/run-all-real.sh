...
# against a REAL enrolled plugin (gateway, devOpen OFF) that was itself enrolled
# to the SAME nats-server via the production createEnrolledNatsConnection path —
# all from ONE shared trust chain — completing an encrypted round-trip. The only
# stand-in left is the echo LLM.
#
...
HOME="$OCH" OPENCLAW_HOME="$OCH" OPENCLAW_DISABLE_BONJOUR=1 \
  ...
  >"$OCH/gateway.log" 2>&1 &
GW_PID=$!
echo "[run-all-real] gateway pid=$GW_PID — waiting for enrollment user_code…"
...
for i in $(seq 1 240); do
  ...
  if ! kill -0 "$GW_PID" 2>/dev/null; then
    echo "[run-all-real] gateway died before enrollment — log:"; cat "$OCH/gateway.log"; exit 2
  fi
  sleep 0.25
done
[ -z "$USER_CODE" ] && { echo "[run-all-real] TIMEOUT waiting for user_code — gateway log:"; cat "$OCH/gateway.log"; exit 2; }
...
echo "[run-all-real] waiting for plugin registration (enrolled NATS connect)…"
for i in $(seq 1 240); do
  if grep -q "\[webchannel\] ✓ NATS mode plugin registered" "$OCH/gateway.log" 2>/dev/null; then
    echo "[run-all-real] gateway ready (enrolled + connected)"
    break
  fi
  if ! kill -0 "$GW_PID" 2>/dev/null; then
