#!/usr/bin/env bash
# Hermetic #87 turn-outcome E2E. Boots the SAME real topology as
# run-enrolled-transport.sh — real device-flow enrollment-server, JWT-auth
# nats-server, a real openclaw gateway running the real plugin, and a
# NKEY-authenticated driver peer that drives the real PoP register hop — but
# points the echo provider at a REJECTION marker so one of the two turns it runs
# fails at the provider.
#
# WHAT IT PROVES (issue #87): `turn_settled{outcome}` is what the browser client
# promotes a user message's `sendState` from — `ok` becomes `completed` (a ✓),
# `error` becomes `failed{reason:"turn-failed"}`, which is what gates the retry
# affordance. A turn rejected by the provider used to settle `ok`, so the widget
# showed a ✓ and no retry for a turn that produced no answer.
#
# This needs a LIVE gateway because the defect lives in a core behavior a unit
# test cannot stage: core does NOT throw on a provider rejection — it absorbs the
# failure and returns its terminal message as an ordinary reply payload flagged
# `isError`, so the plugin's turn resolves cleanly and its `catch` never runs.
# packages/plugin/src/inbound.test.ts pins the plugin-side logic; this pins the
# core behavior that logic reads.
#
# Two turns, ONE gateway boot:
#   (a) a turn carrying $ECHO_FAIL_MARKER → the echo provider answers HTTP 500 →
#       must settle `error` AND still deliver core's terminal message to the user;
#   (b) an ordinary turn → must still settle `ok` (so an unconditional
#       "always error" fix cannot pass this gate).
#
# Everything runs under an isolated OPENCLAW_HOME=/tmp/oc-turn-outcome-e2e (and
...
PEER_ID=turn-outcome-peer
SAAS_ISSUER="https://saas.local/turn-outcome-issuer"

NATS_PID=""; ECHO_PID=""; ISSUER_PID=""; GW_PID=""; ADD_PID=""

cleanup() {
  echo "[run-turn-outcome] cleanup…"
  ...
  if [ -f "$PKG_BAK" ]; then
    cp "$PKG_BAK" "$PKG_JSON"
    rm -f "$PKG_BAK"
    echo "[run-turn-outcome] restored $PKG_JSON"
  fi
}
trap cleanup EXIT
...
ISSUER_PID=$!
echo "[run-turn-outcome] enrollment-server pid=$ISSUER_PID — waiting for JWKS + NATS config…"
for i in $(seq 1 120); do
  if curl -fsS "http://127.0.0.1:$ISSUER_PORT/.well-known/jwks.json" >/dev/null 2>&1 \
     && [ -f "$OCH/operator.jwt" ] && [ -f "$OCH/resolver.json" ]; then
    echo "[run-turn-outcome] issuer ready (JWKS up, NATS config written)"
    break
  fi
  if ! kill -0 "$ISSUER_PID" 2>/dev/null; then
    echo "[run-turn-outcome] enrollment-server died early — log:"; cat "$OCH/issuer.log"; exit 2
  fi
  sleep 0.25
  if [ "$i" -eq 120 ]; then
    echo "[run-turn-outcome] TIMEOUT waiting for issuer — log:"; cat "$OCH/issuer.log"; exit 2
  fi
done
...
NATS_PID=$!
echo "[run-turn-outcome] nats-server pid=$NATS_PID (JWT-auth, ws://127.0.0.1:$NATS_WS) — waiting…"
for i in $(seq 1 120); do
  if grep -q "Server is ready" "$OCH/nats.log" 2>/dev/null; then
    echo "[run-turn-outcome] nats-server ready"
    break
  fi
  if ! kill -0 "$NATS_PID" 2>/dev/null; then
    echo "[run-turn-outcome] nats-server died early — log:"; cat "$OCH/nats.log"; exit 2
  fi
  sleep 0.25
  if [ "$i" -eq 120 ]; then
    echo "[run-turn-outcome] TIMEOUT waiting for nats-server — log:"; cat "$OCH/nats.log"; exit 2
  fi
done
...
ECHO_PID=$!
echo "[run-turn-outcome] echo server pid=$ECHO_PID"
...
if [ -f "$PKG_BAK" ]; then
  echo "[run-turn-outcome] stale $PKG_BAK found — restoring original package.json before re-swapping"
  cp "$PKG_BAK" "$PKG_JSON"
  rm -f "$PKG_BAK"
fi
cp "$PKG_JSON" "$PKG_BAK"
...
' "$PKG_JSON"
echo "[run-turn-outcome] set plugin extensions → ./index-nats.ts"
...
JSON
echo "[run-turn-outcome] wrote $OCH/.openclaw/openclaw.json"
...
# ---------------------------------------------------------------------------
echo "[run-turn-outcome] channels add (config-time device-flow enroll)…"
...
ADD_PID=$!
echo "[run-turn-outcome] channels add pid=$ADD_PID — waiting for enrollment user_code…"
...
for i in $(seq 1 240); do
  ...
  if ! kill -0 "$ADD_PID" 2>/dev/null; then
    echo "[run-turn-outcome] channels add exited before user_code — log:"; cat "$OCH/channels-add.log"; exit 2
  fi
  sleep 0.25
done
[ -z "$USER_CODE" ] && { echo "[run-turn-outcome] TIMEOUT waiting for user_code — channels-add log:"; cat "$OCH/channels-add.log"; exit 2; }
echo "[run-turn-outcome] enrollment user_code=$USER_CODE — approving…"
...
echo "[run-turn-outcome] approve response: $APPROVE"
...
ADD_PID=""
if [ "$ADD_RC" -ne 0 ]; then
  echo "[run-turn-outcome] channels add failed (rc=$ADD_RC) — log:"; cat "$OCH/channels-add.log"; exit 2
fi
CRED_FILE="$(node --import tsx "$REPO/scripts/resolve-storage-path.ts" \
  credentials "$TENANT" "$ACCOUNT_ID" "$OCH")"
[ -f "$CRED_FILE" ] || { echo "[run-turn-outcome] creds NOT persisted at $CRED_FILE — log:"; cat "$OCH/channels-add.log"; exit 2; }
echo "[run-turn-outcome] ✓ credentials persisted at $CRED_FILE"
...
HOME="$OCH" OPENCLAW_HOME="$OCH" OPENCLAW_DISABLE_BONJOUR=1 \
  ...
  >"$OCH/gateway.log" 2>&1 &
GW_PID=$!
echo "[run-turn-outcome] gateway pid=$GW_PID — waiting for structured account readiness (consume persisted creds)…"
for i in $(seq 1 240); do
  ...
  if printf '%s\n' "$LATEST_AGGREGATE" | grep -Eq "event=webchannel\.account_aggregate generation=[^ ]+ state=complete servingCount=1 totalCount=1"; then
    echo "[run-turn-outcome] gateway ready (consumed creds + connected)"
    break
  fi
  if ! kill -0 "$GW_PID" 2>/dev/null; then
    echo "[run-turn-outcome] gateway died early — log:"; cat "$OCH/gateway.log"; exit 2
  fi
  sleep 0.5
  if [ "$i" -eq 240 ]; then
...
# ---------------------------------------------------------------------------
echo "[run-turn-outcome] driving enrolled-transport encrypted round-trip…"
set +e
WEBCHANNEL_GW_URL="http://127.0.0.1:$GW_PORT" \
WEBCHANNEL_NATS_URL="ws://127.0.0.1:$NATS_WS" \
...
WEBCHANNEL_TENANT="$TENANT" WEBCHANNEL_ACCOUNT_ID="$ACCOUNT_ID" WEBCHANNEL_PEER_ID="$PEER_ID" \
  node --import tsx "$REPO/e2e/local/turn-outcome-roundtrip.ts"
RC=$?
set -e

echo "[run-turn-outcome] driver exit code = $RC"
exit "$RC"
