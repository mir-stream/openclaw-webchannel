...
(globalThis as unknown as { runWeb: unknown }).runWeb = async function runWeb(opts: {
  natsUrl: string; jwt: string; agentId: string; tenant: string; peerId: string; text: string;
}): Promise<string> {
  const client = new WebChannelNatsClient({
    url: opts.natsUrl, jwt: opts.jwt, agentId: opts.agentId, tenant: opts.tenant, peerId: opts.peerId,
  });
